...
/** 
 * This is the parse function that iterates the ast. For 
 * every @expose tag a statement is added to the program 
 * parameter and for every @examples tag an element is added 
 * to tests.
 */
...
/**
 * Called on every comment to parse @examples and @expose tags. 
 * @private
 */
function parseComment(comment, append){
  ...
  while(current = splitLines(examples.exec(comment))){
    for(var i in current){
      if(parseExampleLine(current[i][0], current[i][1], test)){
        hastest = true;
      }
    }
  }
...
/**
 * Splits successive lines of the form ... // ... into a left and 
 * right side.
 * @private 
 */
...
/**
 * Build an ast subtree that is used to assign a local variable 
 * to the global scope.
 * @private
 */
...
/**
 * Returns regex string that matches a full comment line
 * @private
 */
...
/**
 * Massive regex string to get all characters allowed in a 
